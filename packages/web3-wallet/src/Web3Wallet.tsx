...
export const Web3Wallet: React.FunctionComponent<Props> = ({address, providerName, networkName, onDisconnect,
  transactions, getAddressLink, ...rest}) => {
      ...
      <Drop plain responsive onClickOutside={() => { if (open) { setJustClosed(true); setOpen(false) }}}
        ...
        {open && <Card>
          ...
          <StatusAddrCopyLink>
            ...
            <Link plain href={getAddressLink(address)} target="_blank"><img src={linkIcon} /></Link>
          </StatusAddrCopyLink>
          <Button label="Disconnect" margin={{ top: '14px' }} onClick={onDisconnect} />
        </Card>}
        {transactions.filter(tx => open ? true : tx.showIfClosed).map(tx =>
          <Card>
            <div>{tx.status}</div>
